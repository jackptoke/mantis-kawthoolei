...
import SelectGradesInputCell from 'components/data-grids/SelectGradesInputCell';
import SubjectEditToolbar from 'components/data-grids/SubjectEditToolbar';

const renderSelectGradesInputCell = (params) => {
  return <SelectGradesInputCell {...params} />;
};
...
const SubjectListPage = () => {
  ...
  const columns = [
    ...
    { field: 'name_eng', headerName: 'Name(English)', width: 200, editable: true },
    {
      ...
      width: 200,
      editable: true,
      renderEditCell: renderSelectGradesInputCell /*valueOptions: grade_levels*/
    },
    {
      field: 'actions',
      type: 'actions',
  ...
  const processRowUpdate = useCallback(
    async (newRow, oldRow) => {
      ...
      if (mutation) {
        ...
        const { data, error } = await supabaseClient.from('Subjects').update(newRow).eq('id', newRow.id).select();
    ...
    <Page title="Subjects List">
      ...
      >
        <DataGridPro
          ...
          getRowHeight={() => 75}
          slots={{ toolbar: SubjectEditToolbar }}
